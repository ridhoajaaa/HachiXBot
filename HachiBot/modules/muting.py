...
from HachiBot import LOGGER, TIGERS, dispatcher
...
def mute(update: Update, context: CallbackContext) -> str:
    ...
    if member.can_send_messages is None or member.can_send_messages:
        ...
        msg = (
            f"Mampus Ngentot! Muted Succses {mention_html(member.user.id, member.user.first_name)} for talking in {chat.title}\n"
...
def unmute(update: Update, context: CallbackContext) -> str:
    ...
    if member.status in ("kicked", "left"):
        ...
        try:
            ...
            pass
        reply = (
            f"Nah! Unmuted Succses {mention_html(member.user.id, member.user.first_name)} "
            f"Oleh {mention_html(user.id, user.first_name)} in <b>{message.chat.title}</b>"
        )
        if reason:
...
def temp_mute(update: Update, context: CallbackContext) -> str:
    ...
    try:
        if member.can_send_messages is None or member.can_send_messages:
            ...
            msg = (
                f"Mampus Ngentot! Temporary Muted {mention_html(member.user.id, member.user.first_name)} from talking for <code>{time_val}</code> in {chat.title}\n"
                f"Oleh {mention_html(user.id, html.escape(user.first_name))}",
            )
